// Library grid with inline-expanding word cards, filterable by status/tier.
import React from "react";
import { T } from "../theme.js";
import { useGame, statusOf } from "../game.jsx";
import { SpeakButton, TierDot, StatusBadge } from "../components/ui.jsx";
...
function ChipList({ items, tone }) {
  return (
    <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
      {items.map((x, i) => (
        <span key={i} style={{
          fontSize: 13.5, padding: "4px 11px", borderRadius: 99,
...
export function WordDetail({ w }) {
  return (
    <div style={{ display: "grid", gap: 18, paddingTop: 18, marginTop: 16, borderTop: `1px solid ${T.lineSoft}` }}>
      <Field label="Meaning">{w.meaning}</Field>
      <Field label="Etymology"><span style={{ color: T.subSolid }}>{w.etymology}</span></Field>
      <Field label="In use">
...
function WordCard({ w, open, onToggle }) {
  const { boxOf } = useGame();
